<x-app-layout>
    <div class="max-w-7xl mx-auto py-10 px-6">
        ...
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-2xl font-bold text-gray-800">Предложения</h1>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @foreach ($offers as $offer)
                            <tr class="hover:bg-gray-50 transition">
                                <td class="px-6 py-4 text-sm text-gray-700">{{ $offer->id }}</td>

                                <td class="px-6 py-4 text-sm text-gray-700">
                                    @if ($offer->img)
                                        <img src="{{ asset('storage/' . $offer->img) }}" alt="Изображение"
                                            class="w-12 h-12 object-contain">
                                    @endif
                                </td>

                                <td class="px-6 py-4 text-sm font-medium text-gray-800">{{ $offer->title }}</td>
                                <td class="px-6 py-4 text-sm text-gray-700">{{ $offer->order }}</td>
                                ...
                                <td class="px-6 py-4 text-right space-x-2">
                                    ...
                                    <form action="{{ route('offer.destroy', $offer) }}" method="POST"
                                        ...
                                        @csrf
                                        @method('DELETE')
                                        <button onclick="return confirm('Удалить это предложение?')"
                        ...
                        @if ($offers->isEmpty())
                            <tr>
                                <td colspan="5" class="px-6 py-6 text-center text-gray-500">Записей пока нет</td>
                            </tr>
                        @endif
                    </tbody>
                </table>
