<x-app-layout>
    <div class="max-w-7xl mx-auto py-10 px-6">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-2xl font-bold text-gray-800">Истории</h1>
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Превью</th>
                            ...
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Медиа</th>
                        ...
                        @foreach ($items as $item)
                            <tr class="hover:bg-gray-50 transition">
                                ...
                                <td class="px-6 py-4 text-sm text-gray-700">
                                    @if ($item->preview)
                                        <img src="{{ asset('storage/' . $item->preview) }}" alt="preview"
                                ...
                                <td class="px-6 py-4 text-sm text-gray-700">
                                    @if ($item->is_active)
                                        <span class="px-2 py-1 text-xs rounded bg-green-100 text-green-700">Да</span>
                                    @else
                                        <span class="px-2 py-1 text-xs rounded bg-gray-100 text-gray-600">Нет</span>
                                    @endif
                                </td>
                                <td class="px-6 py-4 text-sm text-gray-700">{{ $item->media_count }}</td>
                                <td class="px-6 py-4 text-right space-x-2">
                                    <a href="{{ route('stories.media.index', $item) }}"
                                    ...
                                    <a href="{{ route('stories.edit', $item) }}"
                                    ...
                                    <form action="{{ route('stories.destroy', $item) }}" method="POST" class="inline-block">
                                        @csrf
                                        @method('DELETE')
                                        <button onclick="return confirm('Удалить историю?')"
