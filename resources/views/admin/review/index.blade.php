<x-app-layout>
    <div class="max-w-7xl mx-auto py-10 px-6">
        ...
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-2xl font-bold text-gray-800">Отзывы</h1>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @foreach ($items as $item)
                            <tr class="hover:bg-gray-50 transition">
                                ...
                                <td class="px-6 py-4 text-sm">{{ $item->published_at?->format('d.m.Y') }}</td>
                                <td class="px-6 py-4 text-sm">{{ $item->order }}</td>
                                <td class="px-6 py-4 text-right space-x-2">

                                    <a href="{{ route('review.edit', $item) }}"
                                        class="inline-flex px-3 py-1.5 bg-yellow-400 text-white text-xs font-semibold rounded-md hover:bg-yellow-500 transition">
                                        Редактировать
                                    </a>
                                    ...
                                    <form action="{{ route('review.destroy', $item) }}" method="POST"
                                        ...
                                        <button onclick="return confirm('Удалить?')"
                                            class="inline-flex px-3 py-1.5 bg-red-600 text-white text-xs font-semibold rounded-md hover:bg-red-700 transition">
                                            Удалить
                                        </button>
                                    </form>
