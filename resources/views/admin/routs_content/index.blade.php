<x-app-layout>
    <div class="max-w-7xl mx-auto py-10 px-6">
        ...
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-2xl font-bold text-gray-800">Контент маршрутов</h1>

            <a href="{{ route('routsContent.create') }}"
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @foreach ($items as $item)
                            <tr class="hover:bg-gray-50 transition">
                                ...
                                <td class="px-6 py-4 text-sm text-gray-700">{{ $item->route->title ?? '-' }}</td>
                                ...
                                <td class="px-6 py-4 text-right space-x-2">
                                    <a href="{{ route('routsContent.edit', $item) }}"
                                    ...
                                    <form action="{{ route('routsContent.destroy', $item) }}" method="POST"
                                        class="inline-block">
                                        @csrf
                                        @method('DELETE')
