...
<form method="POST" action="{{ $action }}" class="space-y-4 rounded bg-white p-5 shadow">
    ...
    <div><label class="mb-1 block text-sm">Доп. данные (JSON: image, images[], price, button_url, date, features[], advantages[] и т.д.)</label><textarea name="payload_json" rows="12" class="w-full rounded border-gray-300">{{ old('payload_json', json_encode($item->payload, JSON_UNESCAPED_UNICODE|JSON_PRETTY_PRINT)) }}</textarea></div>
